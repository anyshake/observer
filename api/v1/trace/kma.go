...
func (k *KMA) Fetch() ([]byte, error) {
	if time.Since(k.Time) <= EXPIRATION {
		return k.Cache, nil
	}
...
func (k *KMA) Format(latitude, longitude float64, data map[string]any) ([]seismicEvent, error) {
...
func (k *KMA) List(latitude, longitude float64) ([]seismicEvent, error) {
	res, err := k.Fetch()
	if err != nil {
		return nil, err
	}
