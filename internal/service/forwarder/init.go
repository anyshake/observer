...
func (s *ForwarderServiceImpl) Init() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	listenHost, err := (&forwarderConfigListenHostImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
	s.listenHost = listenHost.(string)

	listenPort, err := (&forwarderConfigListenPortImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
