...
func (s *SeedLinkServiceImpl) Init() error {
	s.mu.Lock()
	defer s.mu.Unlock()
	...
	useCompress, err := (&seedlinkConfigCompressImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
	...
	listenPort, err := (&seedlinkConfigListenPortImpl{}).Get(s.actionHandler)
	if err != nil {
		return err
	}
