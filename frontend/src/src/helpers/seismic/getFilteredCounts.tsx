...
export const getFilteredCounts = (record: number[], options: FilterOptions) => {
    const { passbandType, poles, sampleRate, lowFreqCorner, highFreqCorner } =
        options;

    let passband: OregonDSPTop.com.oregondsp.signalProcessing.filter.iir.PassbandType;
    if (passbandType === FilterPassband.LOW_PASS) {
        passband =
            OregonDSPTop.com.oregondsp.signalProcessing.filter.iir.PassbandType
                .LOWPASS;
    ...
    const butterworth =
        new OregonDSPTop.com.oregondsp.signalProcessing.filter.iir.Butterworth(
            poles,
            passband,
            lowFreqCorner,
            highFreqCorner,
